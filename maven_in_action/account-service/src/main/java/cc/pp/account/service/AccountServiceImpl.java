...
import cc.pp.account.captcha.AccountCaptchaException;
import cc.pp.account.captcha.AccountCaptchaService;
import cc.pp.account.captcha.RandomGenerator;
import cc.pp.account.email.AccountEmailException;
import cc.pp.account.email.AccountEmailService;
import cc.pp.account.persist.Account;
import cc.pp.account.persist.AccountPersistException;
import cc.pp.account.persist.AccountPersistService;

public class AccountServiceImpl implements AccountService {

	private final Map<String, String> activationMap = new HashMap<>();
	...
	public byte[] generateCaptchaImage(String captchaKey) throws AccountServiceException {
		...
		try {
			...
			throw new AccountServiceException("Unable to generate Captha Image.", e);
		}
	}

	@Override
	public void singnUp(SignUpRequest signUpRequest) throws AccountServiceException {
