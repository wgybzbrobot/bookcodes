...
public class AccountServiceTest {
	...
	public void testAccountService() throws Exception {
		...
		signUpRequest.setId("maven");
		signUpRequest.setEmail("test@maven.com");
		signUpRequest.setName("mavendemo");
		signUpRequest.setPassword("123");
		signUpRequest.setConfirmPassword("123");
		signUpRequest.setActivateServiceUrl("http://localhost:8080/account/activate");
		accountService.singnUp(signUpRequest);
		...
		try {
			accountService.login("maven", "123");
		...
		// 5. login woth correct id and password
		accountService.login("maven", "123");

		// 5a. login with incorrect password
		try {
			accountService.login("maven", "456");
